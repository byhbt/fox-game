...
function initButton(handleUserAction) {
  ...
  function buttonClick({
    ...
    if (target.classList.contains("left-btn")) {
      ...
      selectedIconIndex = (2 + selectedIconIndex) % _constants.ICONS.length;
      toggleHighlighted(selectedIconIndex, true);
    } else if (target.classList.contains("right-btn")) {
      toggleHighlighted(selectedIconIndex, false);
      selectedIconIndex = (1 + selectedIconIndex) % _constants.ICONS.length;
      toggleHighlighted(selectedIconIndex, true);
    } else {
      handleUserAction(_constants.ICONS[selectedIconIndex]);
    }
  }
...
async function init() {
  console.log("Starting game"); // initButtons(gameState.handleUserAction);
